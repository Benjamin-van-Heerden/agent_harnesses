...
**Key commands:**
- `python .agent_core/harness/main.py spec new "title"` - Create a new spec.
- `python .agent_core/harness/main.py sync issues` - Sync the project context with the remote repository. This is a very important command. It ensures the remote knows about local specs and todos before assignment or GitHub-linked work.
- `python .agent_core/harness/main.py spec assign <slug>` - Assign spec to and create worktree. DO NOT ASSIGN SPECS WITHOUT EXPLICIT CONSENT.
- `python .agent_core/harness/main.py task new <spec_slug> "title" "detailed description with implementation notes if necessary"` - Create a task under a specific spec.
- `python .agent_core/harness/main.py task complete <spec_slug> <task_slug> "detailed notes about what was done"` - Mark task done.
- `python .agent_core/harness/main.py spec complete <slug> "detailed commit message"` - Create PR and mark spec merge-ready.
- `python .agent_core/harness/main.py log new` - Create work log for the session. This is an extremely important command that should be run towards the end of every session. If you feel at any point your context window is becoming too large, suggest creating a log and continuing in a new session.

**Todos (standalone work items):**
- `python .agent_core/harness/main.py todo new "title" "description"` - Create a todo and linked GitHub issue.
- `python .agent_core/harness/main.py todo list` - List all open todos.
- `python .agent_core/harness/main.py todo claim <slug> <user>` - Claim a todo and close the linked GitHub issue.

**Memories (project-specific notes):**
- `python .agent_core/harness/main.py memory new "title" "content"` - Create a memory, a short note about a pattern, convention, or preference.
- `python .agent_core/harness/main.py memory list` - List all memories.
- `python .agent_core/harness/main.py memory show <slug>` - Show memory details.
- `python .agent_core/harness/main.py memory update <slug> "new content"` - Update a memory.
- `python .agent_core/harness/main.py memory delete <slug>` - Delete a memory.
...
- **When the user asks you to remember something** - create a memory with `python .agent_core/harness/main.py memory new "title" "content"`.
...
- `python .agent_core/harness/main.py log new` is not an interactive command. When prompted to "Create a log" or "Let's log", run the log command and follow the instructions.
...
- Do not create `.agent_core/tmp/` during setup.
